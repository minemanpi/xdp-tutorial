...
int udp_server_rx_filter_main(struct xdp_md *ctx) {
    ...
    if (dport == bpf_htons(11211) && payload + 4 <= data_end) {
        if (ip->protocol == IPPROTO_UDP && payload[0] == 'g' &&
            ...
            // These are not working. Why?
            // bpf_printk("UDP Packet: %*pEa", ctx->data_end - (__u32)transp,
            // udp); bpf_printk("Memcached Payload: %*pEa\n",
            //            ctx->data_end - (__u32)payload, payload);

            // Send packet back
            bpf_printk("Returned packet!\n");
            return XDP_TX;
        }
    }
    return XDP_PASS;
